...
// vibrato parameters
double vibratoDepth = 5.0;
double vibratoRate = 6.0;
...
int main(int argc, char *argv[]) {
    ...
    if (!ParseArguments(argc, argv, &output_file_name,
        &waveform, &duration, &frequencies)) {
        return 1;
    }
    ...
    for (double frequency : frequencies) {
        for (int i = 0; i < soundwave::kSampleRate * duration; ++i) {
            ...
            if (t < 0.1 * duration) {  // Fade-in
                ...
                envelope = (duration - t) / (0.1 * duration);
            }

            // Vibrato calculation
            double vibrato =
                vibratoDepth * std::sin(2.0 * M_PI * vibratoRate * t);
            // Add the vibrato effect to the original frequency
            double currentFreq = frequency + vibrato;

            double x = t * currentFreq;
    ...
    for (int i = 0; i < soundwave::kSampleRate * duration; ++i) {
        ...
        buffer[i] = static_cast<int16_t>(std::max(-32768.0,
            std::min(32767.0, val)));
    }
